export class Card {
  ...
  _likeChecking(e, elementsLikeQuantity) {
    if (this._likes.some((el) => el._id === this._userData._id)) {
  ...
  _deleteCard(elementsItem) {
    this._openPopupDeleteImage(() => {
      this._deleteCardServer(this._id).then((res) => {
        elementsItem.remove();
  ...
  _setEventListeners(element, elementsTrashButton, elementsPhoto) {
    const elementLike = element.querySelector(".elements__like");
    const elementsLikeQuantity = element.querySelector(
      ".elements__like-quantity"
    );
    const elementsItem = elementsTrashButton.closest(".elements__item");
    elementsTrashButton.addEventListener("click", () => {
      this._deleteCard(elementsItem);
    });
    elementLike.addEventListener("click", (e) => {
      this._likeChecking(e, elementsLikeQuantity);
    });
  ...
  generateCard() {
    ...
    const elementLikeQuantity = this._element.querySelector(
      ".elements__like-quantity"
    );
    const elementsTrashButton = this._element.querySelector(
    ...
    if (this._owner._id === this._userData._id) {
      elementsTrashButton.style.display = "block";
    }
